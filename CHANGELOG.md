...
### Security
- **`docs.yaml` workflow least-privilege.** Workflow-level `permissions:` block (contents/pages/id-token) split into per-job blocks so the `build` job only gets `contents: read` and only `deploy` gets `pages: write` + `id-token: write`. A future workflow change touching the `build` steps can no longer accidentally write Pages content.
- **Probe TLS verification on by default.** `scripts/verify-api-key-roles` no longer hardcodes `InsecureSkipVerify: true`. Verification is enabled unless the operator opts out with `TRUENAS_INSECURE_SKIP_VERIFY=1` (mirroring the production client's config knob). Both the WebSocket dialer and the `/_upload` HTTP client also pin `MinVersion: TLS 1.2`.
- **Provider chart drops automounted SA token.** `omni-infra-provider-truenas` Helm chart now sets `automountServiceAccountToken: false` on the pod spec. The provider talks Omni gRPC + TrueNAS WebSocket and never reads the Kubernetes API, so the default-mounted token had no legitimate consumer and only widened blast radius if the container were compromised.

### Reliability
- **Deprovision survives ctx cancellation without losing traces.** `internal/provisioner/deprovision.go` zvol-cleanup and force-stop paths now use `context.WithoutCancel(ctx)` instead of `context.Background()`. Cleanup still completes on shutdown but trace IDs / observability metadata propagate from the parent context. Same pattern applied to the autoscaler singleton-lease release in `cmd/omni-infra-provider-truenas/autoscaler.go`.
- **Test context leaks closed.** `deprovision_test.go` and `internal/autoscaler/server_test.go` now `defer cancel()` (or `t.Cleanup(cancel)`) immediately after `context.WithCancel`, so a test that returns early no longer strands the cancel func until GC.

### Post-review polish (parallel-code-review enhancement plan)
- **Probe TLS env parser unified with production.** `scripts/verify-api-key-roles` reads `TRUENAS_INSECURE_SKIP_VERIFY` via `strconv.ParseBool` (`"true"` / `"1"` / `"T"`) — matches the production `envBool` parser. Previously the probe accepted only `"1"` so an operator with `.env: TRUENAS_INSECURE_SKIP_VERIFY=true` would see the probe verify TLS while production silently skipped — false sense of safety.
- **Probe `sync.OnceValue` removed.** The upload HTTP client now takes `insecure bool` as a constructor parameter rather than reading env on first call inside a `sync.OnceValue`. Test ergonomics (`t.Setenv` was order-dependent under the OnceValue) plus pure-constructor semantics.
- **Probe API key zeroized after auth.** `probe.apiKey` is now a `SecretString`; after `auth.login_with_api_key` succeeds the local field is set to zero-value `SecretString{}` and the original env var is wiped. Bearer header for the upload path is rebuilt once and passed as an argument. Core dump / swap leak during the post-auth probes can no longer surface the operator's API key.
- **Probe script over-extractions inlined.** `probeDatasetLifecycle`, `probeVMLifecycle`, `probeVMDeviceCRUD` collapsed back into `main()`. The script's value is top-to-bottom readability for an operator running it once; the helpers added cross-function indirection on a one-shot tool. `dialProbeWebSocket` and `probeReadMethods` (table-driven READ matrix) are kept — those splits paid off.
- **`autoscaler` lease + capacity gate API shape cleanup.** `acquireAutoscalerLease` now returns `(release func(), error)` with a sentinel `errAutoscalerLeaseShutdownDuringAcquire` instead of a tri-state `(release, proceed, err)` return that the caller had to re-decode. `buildAutoscalerCapacityGate` returns a `CapacityGateBundle` with an unconditional `Close()` (no-op when gate is disabled) so the caller can `defer bundle.Close()` without a nil-check dance.
- **`SingletonLeaseHeld` gauge wired on the autoscaler.** Provisioner-side already emitted it; autoscaler was silent. Lease state now visible on Grafana dashboards via `truenas_singleton_lease_held{scope="autoscaler-<cluster>"}`.
...
- **`recordGracefulOutcome` helper inlined; `provider_id` label added.** The 4-line two-arm metric conditional moved back into `cleanupVM` and the counter now carries a `provider_id` attribute so fleet dashboards can segment graceful-shutdown success rate per provider.
- **`attachOneAdditionalNIC` inlined back into `attachAdditionalNICs`.** Splitting one for-loop iteration into a sub-helper made the per-NIC narrative (MAC generation → collision detection → attach → MAC-absent decision → MTU patch append) cross two functions; restored as one transactional unit.
- **`isoCacheRef` constructor.** `newISOCacheRef(dataset, path, imageID)` builds the canonical struct with all three TOFU property names derived from `imageID`. Replaced 12+ hand-filled struct literals (2 production sites + 10 test sites) — a future rename of a property prefix now touches one line instead of twelve.
- **`validateDatasetPrefixSegments` shared.** Both `Validate` (root) and `validateOneAdditionalDisk` (per-disk) now route through a single segment-splitter, parametrized on whether empty segments are rejected. The pre-existing rule-of-three drift (root rejected empty segments, per-disk silently skipped) is now explicit and easy to converge.
- **`runShutdownFuncs` collapsed back into `Init`'s closure.** The 14-line helper had one caller wrapped in a one-call closure — net cog-complexity moved, not reduced. Now an inline `errors.Join` loop in the closure body.
- **`dialCleartextFallback` split fixed.** The cleartext fallback path is inlined back into `dialWebSocket`; only the warn-log decision (loopback suppression) is extracted as `warnCleartextFallback`. The previous helper conflated "format the TLS error" with "re-dial cleartext" under a name that promised the second.
- **`cleanupOrphanVMs::maybeDeleteOrphanVM` records `span.RecordError` on StopVM failure** (was only emitted on DeleteVM failure — half-completed orphan teardown was invisible in trace UI).
- **`metrics.go::contains` documentation honest.** The hand-rolled byte loop → `strings.Contains(ToLower(...), ToLower(...))` swap allocates two strings per call. Comment now flags this as cold-path-only and warns against copying into hot code.
- **`data.go` map size hints.** `seenVolumeNames` and `seen` pre-sized to known upper bounds (`len(d.AdditionalDisks)`, `len(d.AdditionalNICs)+1`) — saves the 1-2 hash-table grows per `Validate` call at MaxAdditional ceilings.
...
- **`internal/client/MockClientCtx`** — new ctx-aware mock transport that exposes `ctx.Err()` to test handlers. Required for the `WithoutCancel` contract pin; reusable for future tests that need to inspect cancellation behavior at the call site.

### Maintainability
- **Large-function refactors.** `stepCreateVM` (provisioner core, previously cog-complexity 129) split into `attachAdditionalDisks`, `attachPrimaryNIC`, `attachAdditionalNICs` + `attachOneAdditionalNIC`, `applyNICConfigPatches`, and `applyAdvertisedSubnetsConfigPatch` — outer function is now a flat sequence of named steps. `stepUploadISO` (cog 56) factored into `downloadOrReuseISO`, `ensureISODatasets`, `downloadAndUploadISO`, and `handleISOHashMismatch` — singleflight closure now ~3 lines. `ensureZvol` (cog 30) extracts the encrypted-existing recovery path into `ensureEncryptedZvol`. `Data.Validate` (cog 81) split into per-domain helpers (`validateCPU`, `validateMemory`, `validateRootAndStorageDisk`, `validatePathNames`, `validateAdditionalDisks` + `validateOneAdditionalDisk`, `validateAdditionalNICs` + `validateOneAdditionalNIC`). `run()` in `cmd/omni-infra-provider-truenas/main.go` (cog 45) extracts `buildLogger` and `acquireProviderLease`. `runAutoscaler` (cog 23) extracts `buildAutoscalerCapacityGate` and `acquireAutoscalerLease`. Provisioner `cleanupVM` (cog 23) extracts `fetchOwnedVM`, `gracefullyStopVM`, and `recordGracefulOutcome`. `scripts/verify-api-key-roles/main.go`'s `main` (cog 30) extracts `dialProbeWebSocket`, `probeReadMethods`, `probeDatasetLifecycle`, `probeVMLifecycle`, and `probeVMDeviceCRUD`. `autoscaler/metrics.go::contains` replaced its hand-rolled case-insensitive byte loop with `strings.Contains(strings.ToLower(...), strings.ToLower(...))` — equivalent semantics, ten lines instead of forty.
- **`isoCacheRef` struct** bundles dataset / path / imageID / TOFU prop names so `verifyCachedISO`, `recordTOFUProperty`, and `setIfPoisonable` each take 4 args instead of 7–9. Test fixtures construct the struct inline; no behaviour change.
...
- **Cognitive complexity reductions on three hot paths.** `dialWebSocket` (`internal/client/ws.go`) extracts the cleartext-fallback branch into `dialCleartextFallback`; `UploadFile` extracts the multipart-pipe writer into `writeUploadMultipart`. `NodeGroupIncreaseSize` (`internal/autoscaler/server.go`) extracts the capacity-gate switch into `Server.evaluateCapacityGate`. `cleanupOrphanVMs` (`internal/cleanup/cleanup.go`) extracts the per-VM orphan-decision body into `Cleaner.maybeDeleteOrphanVM`. `telemetry.Init` extracts its inline shutdown-joiner closure into a top-level `runShutdownFuncs` helper. No behavior change in any of the five — the rule logic moves wholesale into the extracted helper.
- **`pool.dataset.query` literal de-duplication.** Extracted `methodPoolDatasetQuery` + `errPoolDatasetQueryFmt` constants in `internal/client/storage.go`; replaced 7 + 5 inline string usages so a method-name typo or error-format tweak only happens in one place.
- **`scripts/test-longhorn.sh` exit-code correctness.** `log()` / `fail()` switched from `((PASS++))` to `PASS=$((PASS+1)); return 0`. Arithmetic post-increment returns exit status 1 the first time the counter is zero, which made the helper "fail" cleanly on its first call. Behavior is now predictable. `info()`, `cleanup()` similarly return 0 explicitly. Repeated literals (`Succeeded`, `{.status.phase}`, the banner) extracted as readonly consts.
- **`install-longhorn.sh` / `annotate-machineclass-autoscale.sh`** logger helpers assign `$1` to a `local msg` instead of using the positional in-place, matching the shellcheck-recommended style.
- **`Dockerfile` base image.** Dropped the redundant `:nonroot` tag from the FROM line — the digest already pins the image bytes immutably. The `nonroot` context is preserved in the comment above for readers; the uid/gid override below remains the authoritative source for the runtime user.
- **Helm chart ephemeral-storage.** `omni-infra-provider-truenas` values.yaml now declares `requests.ephemeral-storage: 64Mi` + `limits.ephemeral-storage: 256Mi`. Kubelet can size the pod's writable layer + log buffers explicitly instead of falling back to BestEffort eviction.

## [v0.16.2] — Close 6 SAST findings + hardening

### Security
- **GHA tag-name RCE on runner (pre-signed-tag verify).** Release workflow no longer interpolates `${{ github.ref }}` into a shell command before signature verification. Attacker-controlled tag names can no longer execute on the runner.
...
- **/healthz checker rework.** Background refresher with functional options (closes a test-only field-poke seam), every check wrapped in an OTel span, `truenas_healthcheck_errors_total` now increments on every failed check including WS-level and context-deadline failures (previously only incremented inside specific checker subpaths, leaving those failure modes invisible to the existing alert).
...
- New counters `truenas_iso_poison_marker_write_failed_total`, `truenas_iso_poison_marker_retries_total`, `truenas_iso_tofu_metadata_write_failed_total` plus three matching Prometheus alert rules — `MANUAL CLEANUP REQUIRED` no longer relies on log scraping.
- `setIfPoisonable` retries via `time.NewTimer` + `Reset` (no leaked timers on cold ctx-cancel) and returns the persisted-bool so the outer error mentions when the marker write itself failed.
