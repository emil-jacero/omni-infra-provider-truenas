# Agent setup for `omni-infra-provider-truenas`

This is a deepsec scanning workspace. Project `omni-infra-provider-truenas` was just registered
(target: `..`). Setup is incomplete — `data/omni-infra-provider-truenas/INFO.md`
...
2. **Fill in `data/omni-infra-provider-truenas/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id omni-infra-provider-truenas
pnpm deepsec process --project-id omni-infra-provider-truenas
```
