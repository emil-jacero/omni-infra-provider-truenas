...
Currently configured project: `omni-infra-provider-truenas` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/omni-infra-provider-truenas/SETUP.md` to fill in
   `data/omni-infra-provider-truenas/INFO.md`.
...
`--project-id omni-infra-provider-truenas` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/omni-infra-provider-truenas/`.
...
data/omni-infra-provider-truenas/
